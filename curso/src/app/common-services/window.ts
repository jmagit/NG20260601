    ...
    <div class="modal fade show" [style.display]="'block'" tabindex="-1">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div
            class="modal-header bg-gradient text-white"
            [class]="{
          ...
          <div class="modal-body bg-light bg-gradient p-0">
            ...
            @switch (type()) {
              @case (NotificationType.error) {
                <div
                  class="m-0 p-3 alert alert-danger d-flex align-items-center rounded-0"
                  role="alert"
                >
                  <svg
                    class="bi flex-shrink-0 me-2 text-danger"
                    width="24"
                    height="24"
                    role="img"
                    aria-label="Danger:"
              ...
              @case (NotificationType.warn) {
                <div
                  class="m-0 p-3 alert alert-warning d-flex align-items-center rounded-0"
                  role="alert"
                >
                  <svg
                    class="bi flex-shrink-0 me-2 text-warning"
                    width="24"
                    height="24"
                    role="img"
                    aria-label="Danger:"
              ...
              @case (NotificationType.info) {
                <div
                  class="m-0 p-3 alert alert-primary d-flex align-items-center rounded-0"
                  role="alert"
                >
                  <svg
                    class="bi flex-shrink-0 me-2 text-primary"
                    width="24"
                    height="24"
                    role="img"
                    aria-label="Info:"
              ...
              @default {
                <div
                  class="m-0 p-3 alert alert-success d-flex align-items-center rounded-0"
                  role="alert"
                >
                  <svg
                    class="bi flex-shrink-0 me-2 text-success"
                    width="24"
                    height="24"
                    role="img"
                    aria-label="Log:"
          ...
          <div class="modal-footer bg-light bg-gradient">
            @if (isConfirm()) {
              <button type="button" class="btn btn-secondary" (click)="confirmar(true)">
                {{ acceptMsg() }}
              </button>
            }
            <button type="button" class="btn btn-secondary" (click)="confirmar()">
              {{ cancelMsg() }}
            </button>
          </div>
        </div>
...
export class WindowService {
  ...
  alert(message: string, tipo?: NotificationType, titulo?: string, cerrar?: string) {
    ...
    const dialogo = this.view.RootViewContainerRef.createComponent(ConfirmComponent, {
      index: 0,
      bindings: binds,
    });
  }
