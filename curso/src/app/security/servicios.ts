...
export const LOGIN_EVENT = Symbol('login')
export const LOGIN_FORM_OPEN_EVENT = Symbol('login.form.open')
export const LOGIN_FORM_CLOSE_EVENT = Symbol('login.form.close')
export const LOGOUT_EVENT = Symbol('logout')
...
export class AuthService {
  ...
  get isAuthenticated() { return this.isAuth.asReadonly(); }
...
export const AuthCanActivateChild: CanActivateChildFn = (_route, _state) => {
  return inject(AuthService).isAuthenticated();
}
...
export function AuthWithRedirectCanActivate(redirectTo: string): CanActivateFn {
  return (_route, state) => {
    ...
    if (!authService.isAuthenticated() && redirectTo)
      return inject(Router).createUrlTree([redirectTo], { queryParams: { returnUrl: state.url } });
